...
public abstract class AbstractSwerveModule {
    ...
    public AbstractYawEncoder yawEncoder;
     ...
     * subclass.
     *
     * @param gearRatioYaw        The gear ratio, possibly only using a subset of
     *                            the gear pairs in the module, which is responsible
     *                            for the amount of yaw as a function of the two
     *                            motors' speeds
     *
     * @param gearRatioWheelSpeed The gear ratio, typically consisting of all gear
     *                            pairs in the module, responsible for the amount of
     *                            wheel rotation as a function of the two motors'
     *                            speed
     *
     * @param maxYawSpeedRpm      The maximum speed that yaw should be allowed. This
     *                            value is scaled by the encoder output, to adjust
     *                            the yaw of the module
     *
     * @param motorA              Instance of the first of the two motors used for
     *                            differential swerve
     *
     * @param motorB              Instance of the second of the two motors used for
     *                            differential swerve
     *
     * @param encoder             Instance of the encoder used for ascertaining the
     *                            module's yaw
     *
     * @param wheelDiameterMeters The diameter of the module's wheel, in meters
     */
    public void initialize(double gearRatioYaw, double gearRatioWheelSpeed, double maxYawSpeedRpm, IMotor motorA,
            IMotor motorB, AbstractYawEncoder encoder, double wheelDiameterMeters) {
        // Save arguments
        this.motorA = motorA;
        this.motorB = motorB;
        yawEncoder = encoder;
     ...
     * from meters per second and Rotation2d, to revolutions per minute and degrees.
     *
     * @param state The desired module state, with speed in units of meters per
     *              second, and angle in units of radians.
     */
    ...
    public double getYawHeading() {
        return yawEncoder.getHeadingDegrees();
    }
    ...
    /**
     * Function that should be called periodically, typically from `robotPeriodic`
     * or from a Command's `execute` method.
     */
    public void periodic() {
        ...
        // calculatedYawRPM = yawEncoder.getOutputSignedPercent(desiredYawDegrees) *
        // MAX_YAW_SPEED_RPM; //OLD

        double calculatedYawRPM = yawPid.calculate(yawEncoder.getHeadingDegrees(), desiredYawDegrees);
        ...
        if (Robot.debugDash.modTab.overrideFromModuleControlDashboard()) {
            if (Robot.debugDash.modTab.nameMatchesSelectedModule(name)) {
                setMotorSpeedsRPM(desiredWheelSpeedRPM, calculatedYawRPM);
            }
            return;
        }

        // No dashboard overrides in effect
        setMotorSpeedsRPM(desiredWheelSpeedRPM, calculatedYawRPM);
    }
     ...
     *
     * @param aMotorRPM The speed of the first motor, in RPM
     *
     * @param bMotorRPM The speed of the second motor, in RPM
     *
     ...
     * @param bMotorRPM The speed of the second motor, in RPM
     *
     * @return The speed rotation of the module, in RPM
     */
    ...
    /**
     * Calculate the motor A RPM. Calculating motor A and B RPMs require combinding
     * the wheel and yaw RPMs, but the sign of the combination operation varies
     * depending on module design. We therefore let the extending class implement
     * each of these.
     *
     * The implementation will like look like this: return (wheelRPM /
     * GEAR_RATIO_WHEEL_SPEED) + (yawRPM / GEAR_RATIO_YAW); or this: return
     * (wheelRPM / GEAR_RATIO_WHEEL_SPEED) - (yawRPM / GEAR_RATIO_YAW);
     */
    protected double getARPM(double wheelRPM, double yawRPM) {
        return (wheelRPM / GEAR_RATIO_WHEEL_SPEED) - (yawRPM / GEAR_RATIO_YAW);

        // peyton => return (wheelRPM / GEAR_RATIO_WHEEL_SPEED) + (yawRPM /
        // GEAR_RATIO_YAW);
    }

    /**
     * Calculate the motor B RPM. Calculating motor A and B RPMs require combinding
     * the wheel and yaw RPMs, but the sign of the combination operation varies
     * depending on module design. We therefore let the extending class implement
     * each of these.
     *
     ...
     * 
     * @param wheelRPM desired wheel translation in RPM
     * @param yawRPM   desired module yaw in RPM
     */
    ...
    /**
     * Change the PID constants for the Yaw PID
     * 
     * @param kP
     * @param kI
    ...
    public void setYawPIIzDF(double kP, double kI, double kIz, double kD, double kF) {
        ...
        // yawPid.setIntegratorRange(minimumIntegral, maximumIntegral); // this seems to
        // work differently than iZ
    }

    /**
     * Change the Yaw PID max range [NOT YET SUPPORTED]
     * 
     * @param max
     * @param min
     */
