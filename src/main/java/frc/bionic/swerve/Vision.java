...
public class Vision extends SubsystemBase {
    ...
    // The angle of the limelight relative to a paralell ground in relation to the
    // ground
    ...
    /**
     * Gets the distance of the target using the formula from the limelight's
     * documentation.
     *
     * @return The distance to the target using a formula.
     */
    public double calculateDistanceFromCameraHeight(final double targetHeight, final double cameraHeight,
            final double cameraAngle) {
        final double methodDistance = (targetHeight - cameraHeight)
                / Math.tan(Math.toRadians(cameraAngle + getYOffset()));
        return methodDistance;
    }

    /**
     * Gets the distance to the target based of off its area using a slope-intercept
     * formula.
     *
    ...
    void syncShuffleboard() {
        ...
        sb_distance_to_target
                .setDouble(calculateDistanceFromCameraHeight(this.targetHeight, this.cameraHeight, this.cameraAngle));
