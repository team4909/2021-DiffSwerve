...
public class SwerveModule extends AbstractSwerveModule {
    ...
    /**
     * Gear ratio of first two pairs of gears. Yaw does not depend on the third
     * pair.
     */
    ...
    public SwerveModule(int canDeviceChannelA, int canDeviceChannelB, int dioEncoderChannel, String name,
            String shuffleboardTabName) {
        super(name, shuffleboardTabName);

        // Instantiate the two swerve motors our swerve module incorporates
        // true makes the motor go clockwise, if both motors are going clockwise, then
        // positve values = translation
        ...
        AbstractYawEncoder encoder = new YawEncoderRevHex(dioEncoderChannel, name, shuffleboardTabName);

        // Now we have everything our superclass needs to do its thing. Do it!
        initialize(GEAR_RATIO_YAW, GEAR_RATIO_WHEEL_SPEED, MAX_YAW_SPEED_RPM, motorA, motorB, encoder,
